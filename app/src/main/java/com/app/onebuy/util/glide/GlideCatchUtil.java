package com.app.onebuy.util.glide;
...
public class GlideCatchUtil {
    ...
    public String getCacheSize() {
        try {
            return getFormatSizeMBonly(getFolderSize(new File(BasisApp.getInstance().getCacheDir() + "/" + GlideCatchConfig.GLIDE_CARCH_DIR)));
        } catch (Exception e) {
            e.printStackTrace();
            return "0MB";
        }
    ...
    public boolean cleanCacheDisk() {
        return deleteFolderFile(BasisApp.getInstance().getCacheDir() + "/" + GlideCatchConfig.GLIDE_CARCH_DIR, true);
    }
        ...
        try {
            if (Looper.myLooper() == Looper.getMainLooper()) {
                new Thread(new Runnable() {
                    @Override
                    public void run() {
                        Glide.get(BasisApp.getInstance()).clearDiskCache();
                    }
                }).start();
            } else {
                Glide.get(BasisApp.getInstance()).clearDiskCache();
    ...
    public boolean clearCacheMemory() {
        try {
            if (Looper.myLooper() == Looper.getMainLooper()) { //只能在主线程执行
                Glide.get(BasisApp.getInstance()).clearMemory();
                return true;
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    // 格式化单位
    private static String getFormatSizeMBonly(double size) {
        double kiloByte = size / 1024;
//        if (kiloByte < 1) {
//            return size + "Byte";
//        }
